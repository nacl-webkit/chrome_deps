...
gboolean messageCallback(GstBus*, GstMessage* message, AudioStreamGStreamer* destination)
{
    return destination->handleMessage(message);
}

#ifndef GST_API_VERSION_1
static void onGStreamerWavparsePadAddedCallback(GstElement*, GstPad* pad, AudioStreamGStreamer* destination)
{
    destination->finishBuildingPipelineAfterWavParserPadReady(pad);
}
#endif

AudioStreamGStreamer::AudioStreamGStreamer(AudioOutputStream::AudioSourceCallback* callback, float sampleRate, AudioBus* renderBus)
    ...
    , m_isPlaying(false)
{
    ...
    GstElement* webkitAudioSrc = reinterpret_cast<GstElement*>(g_object_new(WEBKIT_TYPE_PEPPER_SRC,
                                                                            "rate", sampleRate,
                                                                            "bus", renderBus,
                                                                            "provider", callback,
                                                                            "frames", renderBus->frames(), NULL));
...
AudioStreamGStreamer::~AudioStreamGStreamer()
{
    GRefPtr<GstBus> bus = webkitGstPipelineGetBus(GST_PIPELINE(m_pipeline));
    ASSERT(bus);
...
void AudioStreamGStreamer::finishBuildingPipelineAfterWavParserPadReady(GstPad* pad)
{
...
gboolean AudioStreamGStreamer::handleMessage(GstMessage* message)
{
...
void AudioStreamGStreamer::start()
{
    ASSERT(m_wavParserAvailable);
    if (!m_wavParserAvailable)
        return;
...
void AudioStreamGStreamer::stop()
{
